...
const decisionReason = reasonArg ? reasonArg.split('=').slice(1).join('=') : null;
...
task.status = approve ? 'pending' : 'blocked';
task.approvalResolvedAt = new Date().toISOString();
task.approvalDecisionReason = decisionReason;
if (approve) task.resumeRequested = true;
fs.writeFileSync(taskPath, JSON.stringify(task, null, 2), 'utf8');

const approvalResult = resolveApproval(baseDir, taskId, approve, decisionReason);
console.log(JSON.stringify({ taskId, target, approved: approve, approvalResult, resumeRequested: approve, decisionReason }, null, 2));
